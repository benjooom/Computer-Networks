...
from . import comm_tcp as interface
...
def _getByName (name):
  return topoOf(sys.modules["builtins"].__dict__.get(name, None))


"""Simulator's global IP table"""
sim_ip_to_entry_table = dict()
default_ip_base = "192.168.0.1"
def NextIpAddr ():
  """
  Simply find the largest value in the iptable and return next available address
  """
  addr = [int(x) for x in default_ip_base.split(".")]
  for key, value in sim_ip_to_entry_table.items():
    t_addr = [int(x) for x in key.split(".")]
    for idx in range(0, 4):
      if t_addr[idx] > addr[idx]:
        addr = t_addr
        break
      elif t_addr[idx] < addr[idx]:
        break
  # Next address
  addr[3] += 1
  for idx in range(3, -1, -1):
    if addr[idx] >= 255:
      if idx == 0:
        raise ValueError("IP address should not be larger than 254.255.255.255")
      addr[idx] = 1
      continue
    else:
      break
...
""" Find and return entry from simulator's global IP table"""
def GetEntryFromIpAddr(ip_addr):
  if ip_addr in sim_ip_to_entry_table:
    # simlog.info("IP to Entry: " + ip_addr + " to " + str(sim_ip_to_entry_table[ip_addr]))
    return sim_ip_to_entry_table[ip_addr]
  else:
    return None
...
def CreateEntity (_name, _kind, *args, **kw):
  ...
  """
  if _name in sys.modules["builtins"].__dict__:
    raise NameError(str(_name) + " already exists")
  from . import api
  # Handle IP addresses
  if not "ip_addr" in kw:
    ip_addr = NextIpAddr()
  else:
    ip_addr = kw['ip_addr']
    del kw['ip_addr']
  if ip_addr in sim_ip_to_entry_table:
    raise NameError("Ip address " + str(ip_addr) + " already exists")
  # Create entry
  e = _kind(*args, **kw)
  ...
  if hasattr(e, 'num_ports'):
    ports = e.num_ports
    growPorts = False
  sim_ip_to_entry_table[ip_addr] = e
  simlog.info("New ip address assigned: " + ip_addr + " for " + str(e))
  setattr(e, 'ip_addr', ip_addr)
  ...
  def remove ():
    ...
    try:
      del sys.modules["builtins"].__dict__[_name]
    except:
      pass
  ...
  sys.modules["builtins"].__dict__[_name] = e
