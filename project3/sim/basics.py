...
class BasicHost (HostEntity):
    ...
    def send_data(self, packet):
        """
        Sends a packet
        - Here we assume this host is always connected to
          a single router. So, simply broadcast the packet.
        """
...
class DNSPayload (Packet):
    """
    A DNS request payload from the DNS application
    """
    ReqStr = str("request")
    ResFoundHostStr = str("response_found_host")
    ResFoundDNSStr = str("response_found_dns_server")
    ResNotFoundStr = str("response_not_found")

    def __init__(self, opcode=ReqStr, dns_ttl=time.time(), request=None, response=None):
        Packet.__init__(self)
        self.opcode = opcode
    ...
    def __repr__(self):
        return ("<OP: %s | Req: %s | Res: %s from %s->%s>"
                % (str(self.opcode), str(self.request), str(self.response),
                   self.src.name if self.src else None,
                   self.dst.name if self.dst else None))
